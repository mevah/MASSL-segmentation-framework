...
Date: 22 Jan 2019
"""
import Network_training_SSL_epoch_attention_BraTS18
    ...
    for job in job_list:
        #for split in data_split_list[:]:
            ...
            for seed in data_seed_list:
                ...
                val_dice, test_results = Network_training_SSL_epoch_attention_BraTS18.network_training_ssl_epoch(Test_only, job, seed, split, device,
                ...
                ###################  val each seed
                file = open(str(folder_name) + str(job) + '/' + str(split) + '/seed' + str(seed) + '/val_dice_seed_' + str(seed) + '.txt', 'w')
                title = 'Dice\n'
                history = (
                    '{:4f}\n'
                ...
                ####################   test each seed
                file = open(str(folder_name) + str(job) + '/' + str(split) + '/seed' + str(seed) + '/test_results_seed_' + str(seed) + '.txt', 'w')
...
print('All PC SSL_BraTS18 attention jobs finished')
